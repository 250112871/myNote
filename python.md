...
18|os.fstatvfs(fd)|返回包含文件描述符fd的文件的文件系统的信息，像 statvfs(19\|os.fsync(fd)\|强制将文件描述符为fd的文件写入硬盘。
...
36|os.mkfifo(path[, mode])|创建命名管道，mode 为数字，默认为 0666 (八进制37\|os.mknod(filename[, mode=0600, device]创建一个名为filename文件系统节点（文件，设备特别文件或者命名pipe）。
...
>>>>>>> d51bdfbf74af30da667a75a78ec2bccb0434b816
